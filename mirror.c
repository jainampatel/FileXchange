...
int main()
{
    ...
    // created client socket arr for multiple clients
    int mirrorSocket, clientSocketsArr[ALLOWED_CLIENTS] = {0};
    // created mirror and client obj
    struct sockaddr_in mirrorObj, clientObj;
    socklen_t addrSize = sizeof(struct sockaddr_in);

    // initailized socket for mirror
    if ((mirrorSocket = socket(AF_INET, SOCK_STREAM, 0)) == -1)
    {
        perror("Error creating socket");
        exit(EXIT_FAILURE);
    }
    // added this reuse of port
    int reuse = 1;
    if (setsockopt(mirrorSocket, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(int)) == -1)
    {
    ...
    mirrorObj.sin_family = AF_INET;
    mirrorObj.sin_addr.s_addr = INADDR_ANY;
    mirrorObj.sin_port = htons(MIRRORPORT);

    // bind the socket
    if (bind(mirrorSocket, (struct sockaddr *)&mirrorObj, sizeof(mirrorObj)) == -1)
    {
        perror("Error binding socket");
        exit(EXIT_FAILURE);
    }

    // listening to socket with limit of 100
    if (listen(mirrorSocket, ALLOWED_CLIENTS) == -1)
    {
    ...
    {
        if ((clientSocketsArr[connectedClients] = accept(mirrorSocket, (struct sockaddr *)&clientObj, &addrSize)) == -1)
        {
...
void pclientrequest(int clientSocket)
{
    ...
    while (1)
    {
        ...
        if (strncmp(commandEnteredByUser, "quitc", 5) == 0)
        {
...
int perfromSearchAndGetInfo(int clientSocket, const char *filename, char *result)
{
    ...
    char findCommand[ALLOWED_MESSAGE_SIZE];
    snprintf(findCommand, sizeof(findCommand), "find ~/UOW -type f -name %s", filename);

    FILE *fp = popen(findCommand, "r");
    if (fp == NULL)
    {
        perror("popen");
        return 0;
...
int performSearchAndCompressByCommands(int clientSocket, char commandEnteredByUser[])
{
    ...
    if (strncmp(commandEnteredByUser, "getfz", 5) == 0)
    {
        ...
        snprintf(findCommand, sizeof(findCommand), "find ~/UOW -type f -size +%ldc -size -%ldc -exec tar -rf temp.tar.gz --no-recursion {} + | gzip > temp.tar.gz", size1, size2);
    }
    else if (strncmp(commandEnteredByUser, "getft", 5) == 0)
    {
        ...
        while (token != NULL)
        {
            if (i == 0)
            {
                snprintf(findCommand, sizeof(findCommand), "find ~/UOW -type f \\( -name '*.%s' ", token);
            }
            else
            {
        ...
        snprintf(findCommand + strlen(findCommand), sizeof(findCommand) - strlen(findCommand), "\\) -exec tar -rf temp.tar.gz {} +");
    }
    else if (strncmp(commandEnteredByUser, "getfdb", 6) == 0)
    {
        ...
        snprintf(findCommand, sizeof(findCommand), "find ~/UOW -type f ! -newermt %s -exec tar -rf temp.tar.gz {} +", date);
    }
    else if (strncmp(commandEnteredByUser, "getfda", 6) == 0)
    {
        char date[ALLOWED_MESSAGE_SIZE];
